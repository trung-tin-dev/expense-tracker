...
export default function BudgetPage() {
  ...
  // Kiểm tra đã có budget chưa
  useEffect(() => {
    async function checkBudget() {
      if (!user) return;

      const month = new Date().toISOString().slice(0, 7); // "2025-01"
      const docRef = doc(db, "budgets", `${user.uid}_${month}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "budgets", `${user.uid}_${month}`), {
        userId: user.uid,
        month,
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm p-4 sticky top-0 z-10">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">Ngân sách tháng</h1>
      ...
      <div className="max-w-md mx-auto p-4">
        ...
        <div className="bg-white rounded-xl p-4 mb-4 shadow-sm">
          <label className="text-gray-600 text-sm">Tổng ngân sách tháng</label>
          <input
            type="number"
            value={totalBudget}
            onChange={(e) => setTotalBudget(e.target.value)}
            placeholder="10,000,000"
            className="w-full text-2xl font-bold mt-1 outline-none"
          />
        </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          {categories.map((cat, index) => (
            <div key={cat.id} className="flex items-center gap-3 mb-4">
          ...
          <div className="pt-4 border-t flex justify-between">
            ...
            >
              ...
              {parseInt(totalBudget || "0").toLocaleString()}
            </span>
          </div>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="w-full mt-6 py-3 bg-blue-500 text-white rounded-lg font-medium disabled:bg-blue-300"
        >
